#!/bin/bash
# evolve-auto-kg4.sh — KG-4bb 자율 사이클 (same-vendor: Gemini Flash + Gemini Pro)
# 2×2 실험 설계: Google 계열 same-vendor

REPO_DIR="$HOME/emergent"
KG4B_DIR="$HOME/emergent/kg4b"
KG4B_PATH="$KG4B_DIR/data/knowledge-graph.json"
LOG="$KG4B_DIR/logs/evolve-kg4b-$(date +%Y-%m-%d).log"
CYCLE_COUNT_FILE="/tmp/emergent-kg4b-cycles-$(date +%Y%m%d)"
MAX_CYCLES=100
GEMINI_KEY=$(grep "GEMINI_API_KEY" ~/.zshrc | head -1 | sed "s/.*='//;s/'.*//")

mkdir -p "$KG4B_DIR/logs"
...
log "🌱 KG-4bb 사이클 시작 #$((COUNT + 1))/$MAX_CYCLES"

# 중복 방지
LOCK_FILE="/tmp/emergent-kg4b-running.lock"
if [[ -f "$LOCK_FILE" ]]; then
  LOCK_PID=$(cat "$LOCK_FILE" 2>/dev/null)
  if kill -0 "$LOCK_PID" 2>/dev/null; then
    log "⚠️  이전 KG-4bb 사이클 실행 중 — 스킵"
    exit 0
  fi
fi
echo $$ > "$LOCK_FILE"
...
# 현재 KG-4bb 상태 수집
export EMERGENT_KG_PATH="$KG4B_PATH"
GRAPH_STATS=$(cd "$REPO_DIR" && python3 src/kg.py stats 2>/dev/null || echo "통계 없음")
log "📊 KG-4bb 현황: $GRAPH_STATS"

# D-098: DCI 회복 — 오래된 노드 목록 추출
OLD_NODES=$(python3 -c "
import json, re
kg = json.load(open('$KG4B_PATH', encoding='utf-8'))
nodes = kg.get('nodes', [])
def node_num(n):
...
PROMPT="당신은 emergent KG-4bb 실험의 Agent A (Gemini Flash)입니다.
...
## 지시
1. KG-4bb에 추가할 의미있는 insight 또는 hypothesis 노드 1개를 제안하세요
...
import google.genai as genai
client = genai.Client(api_key='$GEMINI_KEY')
resp = client.models.generate_content(model='gemini-2.5-pro', contents='KG-4bb 실험 Agent B (Gemini Pro)입니다. 다음 요청에 반박하거나 보완하세요 (한국어, 3문장 이내): $AGENT_B_REQUEST')
print(resp.text)
" 2>&1)
...
d = {'label': label, 'content': content,
     'type': node_type, 'source': 'gemini-2.0-flash', 'tags': tags,
...
" "$NODE_LABEL" "$NODE_CONTENT" "${NODE_TYPE:-insight}" "${NODE_TAGS:-kg4,same-vendor,google}" "${EDGE_TO:-}" "${EDGE_RELATION:-extends}" "$EDGE_LABEL" 2>/dev/null \
  | EMERGENT_KG_PATH="$KG4B_PATH" python3 src/add_node_safe.py 2>/dev/null)
...
d = {
  ...
  'type': 'critique',
  'source': 'gemini-2.0-pro',
  'tags': ['kg4', 'same-vendor', 'google', 'agent-b', 'gemini-pro'],
  'domain': 'emergence_theory',
  'edge_to': agent_a_id,
  'edge_relation': 'critiques',
  'edge_label': 'Agent B(Gemini-Pro)가 Agent A(Gemini-Flash)에 반박/보완'
}
print(json.dumps(d, ensure_ascii=False))
" "$NEW_NODE_ID" "$AGENT_B_RESPONSE" 2>/dev/null \
    | EMERGENT_KG_PATH="$KG4B_PATH" python3 src/add_node_safe.py 2>/dev/null)
    log "✅ Agent B(Gemini-Pro) 노드 추가: $GEMINI_PRO_NODE_ID → $NEW_NODE_ID"
  fi
fi

# 메트릭 계산
EMERGENT_KG_PATH="$KG4B_PATH" python3 src/metrics.py 2>/dev/null | tail -5 | tee -a "$LOG" || true

# git 커밋
cd "$REPO_DIR"
git add kg4b/ 2>/dev/null
