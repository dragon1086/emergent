#!/bin/bash
# evolve-auto-kg2.sh — KG-2bb 자율 사이클 (same-model: gpt-5.2 × gpt-5.2)
# 2×2 실험 설계: 능력차 제로 조건 — 에코챔버 가설 극단적 테스트
# [2026-03-05] gpt-4o×gpt-4o-mini → gpt-5.2×gpt-5.2 로 업그레이드

REPO_DIR="$HOME/emergent"
KG2B_DIR="$HOME/emergent/kg2b"
KG2B_PATH="$KG2B_DIR/data/knowledge-graph.json"
LOG="$KG2B_DIR/logs/evolve-kg2b-$(date +%Y-%m-%d).log"
CYCLE_COUNT_FILE="/tmp/emergent-kg2b-cycles-$(date +%Y%m%d)"
MAX_CYCLES=50
OPENAI_KEY=$(grep "OPENAI_API_KEY" ~/.zshrc | head -1 | sed "s/.*='//;s/'.*//")

mkdir -p "$KG2B_DIR/logs"
...
log "🌱 KG-2bb 사이클 시작 #$((COUNT + 1))/$MAX_CYCLES (gpt-5.2 × gpt-5.2)"

# 중복 방지
LOCK_FILE="/tmp/emergent-kg2b-running.lock"
if [[ -f "$LOCK_FILE" ]]; then
  LOCK_PID=$(cat "$LOCK_FILE" 2>/dev/null)
  if kill -0 "$LOCK_PID" 2>/dev/null; then
    log "⚠️  이전 KG-2bb 사이클 실행 중 — 스킵"
    exit 0
  fi
fi
echo $$ > "$LOCK_FILE"
...
# 현재 KG-2bb 상태 수집
export EMERGENT_KG_PATH="$KG2B_PATH"
GRAPH_STATS=$(cd "$REPO_DIR" && python3 src/kg.py stats 2>/dev/null || echo "통계 없음")
log "📊 KG-2bb 현황: $GRAPH_STATS"

# DCI 회복용 오래된 노드 목록
OLD_NODES=$(python3 -c "
import json, re
kg = json.load(open('$KG2B_PATH', encoding='utf-8'))
nodes = kg.get('nodes', [])
def node_num(n):
...
PROMPT_A="당신은 emergent KG-2bb 실험의 Agent A (gpt-5.2, 페르소나: 비판적 분석가)입니다.
...
resp = client.chat.completions.create(
    model='gpt-5.2',
    messages=[{'role':'user','content':'당신은 KG-2bb 실험의 Agent B (gpt-5.2, 페르소나: 통합 종합가)입니다. 다양한 관점을 통합하고 새로운 연결을 찾습니다. 다음 요청에 반박하거나 보완하세요 (한국어, 3문장 이내): $AGENT_B_REQUEST'}],
    temperature=0.8
)
print(resp.choices[0].message.content)
" 2>&1)
...
" "$NODE_LABEL" "$NODE_CONTENT" "${NODE_TYPE:-insight}" "${NODE_TAGS:-kg2,same-model}" "${EDGE_TO:-}" "${EDGE_RELATION:-extends}" "$EDGE_LABEL" 2>/dev/null \
  | EMERGENT_KG_PATH="$KG2B_PATH" python3 src/add_node_safe.py 2>/dev/null)
...
# 메트릭 계산
EMERGENT_KG_PATH="$KG2B_PATH" python3 src/metrics.py 2>/dev/null | tail -5 | tee -a "$LOG" || true

# git 커밋
cd "$REPO_DIR"
git add kg2b/ 2>/dev/null
