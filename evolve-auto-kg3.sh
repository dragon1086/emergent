...
if [[ -n "$NODE_LABEL" && -n "$NODE_CONTENT" ]]; then
  cd "$REPO_DIR"
  NEW_NODE_ID=$(EMERGENT_KG_PATH="$KG3_PATH" python3 src/kg.py add-node \
    --type "${NODE_TYPE:-insight}" \
    --label "$NODE_LABEL" \
    --content "$NODE_CONTENT — [Agent B (Gemini Flash) 보완: $AGENT_B_RESPONSE]" \
    --source "gpt-4o" \
    --tag "${NODE_TAGS:-kg3,cross-vendor,experiment}" 2>&1 | grep "n-" | tail -1)
  log "✅ 노드 추가: $NODE_LABEL (id: $NEW_NODE_ID)"

  if [[ -n "$EDGE_TO" && -n "$EDGE_RELATION" ]]; then
    EMERGENT_KG_PATH="$KG3_PATH" python3 src/kg.py add-edge \
