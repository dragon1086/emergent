...
\begin{abstract}
In the process of two AI agents co-evolving through 80 conversational cycles via a shared
knowledge graph (KG), structural patterns that were not designed in advance repeatedly
emerged. This study defines this phenomenon as \textbf{Inter-Agent Emergence} and proposes
an integrated theoretical framework (5-layer) covering conditions, measurement, design,
universality, and paradoxes of emergence.

\medskip\noindent\textbf{Core contributions:}
\begin{enumerate}
  \item \textbf{Measurement framework}: $E_{v4}$ formula + 4 metrics
        (CSER/DCI/edge\_span/node\_age\_div)
  \item \textbf{Paradoxical Emergence (D-063)}: counter-intuitive crossings (span$\geq$50)
        generate stronger emergence than predictable ones ---
        120 empirically confirmed instances
  ...
  \item \textbf{Design tool}: pair\_designer algorithm --- calculates optimal initial
        conditions for emergence (v4: D-065 paradox resolved, $\Delta$ expanded $3\times$)
  \item \textbf{Robustness validation (D-068)}: 94\% robust across 16 weight-variation
        scenarios ($\pm$20\% perturbation)
  \item \textbf{H\_exec gate mechanism (Cycles 78--79)}: CSER $<$ 0.30 is a hard execution
        barrier --- echo-chamber collaboration cannot produce code at all
        (A: 5/5 pass, B: 0/3 blocked, C: 0/3 blocked)
  \item \textbf{Observer Non-Independence (D-047) empirical confirmation (Cycle 80)}:
        running the execution loop itself reverses $E_{v4}$
        (0.4616 $\to$ 0.4287) by forming short-span edges --- the measurement act
        modifies the substrate it measures
\end{enumerate}

\medskip\noindent
Current KG state (Cycle 80): \textbf{256 nodes / 919 edges}, CSER=0.8009
(echo-chamber escape confirmed: CSER $>$ 0.5).
...
\begin{itemize}
  \item \textbf{Condition A} (this study, executed): Asymmetric persona.
        CSER=0.8009, $E_{v4}$=0.4401
  \item \textbf{Condition B} (future): Symmetric persona (predicted CSER $<$ 0.5)
