...
\date{Draft v3.8 — Latest Snapshot (2026-03-05)}
...
initiated 2026-03-05); within-cell replication is ongoing.
...
\begin{enumerate}
  \item \textbf{Sample size ($N=4$ KG instances, $N=1$ per cell)}: Primary structural
        ...
        KG-2 & GPT-5.2 $\times$ GPT-5.2 & Same-model (OpenAI) & 112 & 0.545$^{**}$ \\
        ...
        Early trajectory (67 nodes, 112 edges): CSER declined from 1.00 (cycle 1)
        to 0.545 (current), consistent with progressive echo-chamber pressure under
        identical-model pairing. Prior 30-cycle snapshot (CSER $=0.83$, GPT-4o $\times$
        GPT-4o-mini) is retained as a supplementary data point.}
        \end{table}

        \noindent\textbf{Key replication finding ($N=4$ KG instances; KG-2: early trajectory, KG-3/4: 130+ cycles)}:
        \begin{enumerate}
          \item The gate-passing CSER result replicates across KG-main, KG-2, and KG-3
                ...
                (67 nodes), suggesting same-model pairs converge even at highest model tier.
...
\begin{itemize}
  \item \textbf{Within-cell replication (ongoing)}: Three replication-B instances
        (KG-2b: GPT-5.2$\times$GPT-5.2, KG-3b: GPT$\times$Gemini, KG-4b: Gemini$\times$Gemini)
        were initiated 2026-03-05 and are accumulating cycles via automated nightly runners.
        Full within-cell trajectory comparison (original vs.\ replication-B) will be
        reported once each reaches $\geq$30 cycles.
