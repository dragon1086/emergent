...
\begin{enumerate}
  \item \textbf{Sample size ($N=1$ system)}: This is a single-pair, single-KG, single-run
        experiment. \emph{All structural findings in this paper --- CSER trajectories,
        gate thresholds, paradoxical emergence statistics, $E_{v4}$ values --- are
        observations from this one experimental instance.} No claim generalizes beyond
        this context without independent replication.
        \textit{Partial resolution (Cycle 86 snapshot)}: The heterogeneous LLM pair
        ...
        (CSER = 0.5455, gate passed). This does not resolve the fundamental N=1
        constraint but demonstrates that the gate mechanism is not unique to the
        original agent pair.
        ...
        \end{enumerate}
        \noindent These findings constitute exploratory replications ($N=1$ KG per
        configuration) and do not resolve the fundamental single-system limitation,
        but provide directional evidence that the CSER gate mechanism is not
        unique to the original agent pair.
...
Across 89+ cycles, this case study provides empirical support for the five-layer
...
\noindent\textbf{Scope statement}: All KG structural findings above derive from a
single $N=1$ two-agent, single-KG experiment. The amplification benchmark
($N=10$ questions, one cross-provider configuration, single judge) and the
supplementary controlled experiments (Cycles 90--92) are distinct from the
main KG case study and carry their own, narrower $N$-constraints; they should
not be interpreted as independent replications of the $N=1$ KG findings.
