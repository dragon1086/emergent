...
\begin{enumerate}
  \item \textbf{Sample size ($N=1$ system)}: This is a single-pair, single-KG, single-run
        ...
        KG-2 & GPT-4o $\times$ GPT-4o-mini & Same (OpenAI) & 18$^*$ & 0.8889$^*$ \\
        KG-3 & GPT-4o $\times$ Gemini-2.0-Flash & Cross & 13$^*$ & 0.9231$^*$ \\
        KG-4 & Gemini-Flash $\times$ Gemini-Pro & Same (Google) & 15$^*$ & 0.4667$^*$ \\
        \bottomrule
        \end{tabular}
        \caption*{\small $^*$Preliminary snapshot (cycle $\leq 15$, 2026-03-04).
        All four instances exceed the gate threshold ($\theta=0.30$).
        KG-4 (same-vendor) shows lower CSER than cross-vendor instances,
        consistent with the persona-diversity hypothesis.
        Target: $\geq 30$ cycles per instance for reportable comparison.}
        \end{table}

        This early evidence suggests that the main KG's gate-passing CSER
        replicates across different agent configurations.
        Preliminary pattern: KG-4 (same-vendor Google: Gemini Flash $\times$ Pro)
        shows notably lower CSER (0.47) than the cross-vendor instances (0.85--0.92),
        while KG-2 (same-vendor OpenAI: GPT-4o $\times$ GPT-4o-mini) shows
        unexpectedly high CSER (0.89), suggesting that within-vendor CSER
        may depend on capability gap rather than vendor identity alone.
        These patterns are \emph{very preliminary} ($\leq 18$ edges per instance)
        and require $\geq 30$ cycles for interpretable comparison.
        Full results will be reported in a follow-up once $\geq 30$ cycles
        are completed per instance.
...
\begin{enumerate}
  \item \textbf{KG replication ($2\times2$ design, in progress)}: KG2/KG3/KG4
        instances (initialized 2026-03-04) are being evolved in parallel.
        KG2 (GPT-4o + GPT-4o-mini, same-vendor) and KG4 (Gemini Flash + Pro,
        same-vendor) will serve as same-vendor controls; KG3 (GPT-4o + Gemini,
        cross-vendor) will test whether the main KG CSER dynamics replicate
        under a different cross-vendor pair. Target: $N=4$ independent KG
        instances with $\geq 30$ cycles each.
