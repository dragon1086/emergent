...
\begin{enumerate}
  \item \textbf{Sample size ($N=1$ system)}: This is a single-pair, single-KG, single-run
        ...
        KG-2 & GPT-4o $\times$ GPT-4o-mini & Same (OpenAI) & 13$^*$ & 0.9231$^*$ \\
        KG-3 & GPT-4o $\times$ Gemini-2.0-Flash & Cross & 9$^*$ & 0.8889$^*$ \\
        KG-4 & Gemini-Flash $\times$ Gemini-Pro & Same (Google) & 11$^*$ & 0.5455$^*$ \\
        \bottomrule
        \end{tabular}
        ...
        replicates across different agent configurations, and that same-vendor
        pairs (KG-2, KG-4) may exhibit systematically lower CSER than
        cross-vendor pairs (KG-3), consistent with the theoretical prediction
        that architectural heterogeneity drives cross-source emergence.
