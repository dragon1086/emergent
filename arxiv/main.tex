...
\begin{enumerate}
  ...
  \item \textbf{$2\times2$ vendor-diversity replication}: Four independent KG instances
        ...
        A nuance: same-model pairs with persona opposition (KG-2, CSER$=0.534$) can
  ...
  \item \textbf{Within-cell replication signal}: Independent replication-B instances
        ...
        KG-2b CSER$=0.513$ tracks KG-2 CSER$=0.534$ (same declining trajectory, $N=72$ nodes),
...
KG-2 early trajectory (GPT-5.2 $\times$ GPT-5.2, 95 nodes): CSER $= 0.534$, declining from 1.00
...
KG-2b CSER (0.513) tracks KG-2 (0.534) on the same declining trajectory, providing
...
were initiated 2026-03-05; KG-2b (72 nodes, CSER$=0.513$) tracks KG-2
(95 nodes, CSER$=0.534$) on the same echo-chamber trajectory.
...
\begin{itemize}
  ...
  \item \textbf{Vendor-diversity ordering}: CSER correlates with agent heterogeneity
        (KG-main$=0.851>$KG-2$=0.534>$KG-3$=0.380>$KG-4$=0.254$), with a
        persona$\times$vendor interaction: same-model persona-diverse pairs can exceed
        cross-vendor same-persona pairs
  \item \textbf{Within-cell replication}: Independent KG-2b instance (72 nodes) tracks
        KG-2 (95 nodes) on the same declining CSER trajectory ($\Delta=0.02$),
...
\begin{enumerate}
  \item \textbf{Sample size ($N=4$ KG instances, $N=1$ per cell)}: Primary structural
        ...
        (replication-B instances: KG-2b CSER$=0.513$ (tracking KG-2\,0.534), KG-3b CSER$=0.475$, KG-4b CSER$=0.543$; all at 72 nodes, 2026-03-05).
        ...
        KG-2 & GPT-5.2 $\times$ GPT-5.2 & Same-model (OpenAI) & 161 & 0.534$^{**}$ \\
        ...
        Early trajectory (95 nodes, 161 edges): CSER declined from 1.00 (cycle 1)
        to 0.534 (current), consistent with progressive echo-chamber pressure under
        identical-model pairing. Replication-B (KG-2b, 72 nodes, CSER $=0.513$) tracks the same declining
        ...
        \begin{enumerate}
          \item The gate-passing CSER result replicates across KG-main, KG-2, and KG-3
                ...
                echo-chamber pressure: CSER declined from 1.00 (cycle~1) to 0.534
                (95 nodes). Replication-B (KG-2b, 72 nodes, CSER $=0.513$) tracks this declining trajectory---first within-cell replication of the same-model
                echo-chamber effect.
          \item Across-vendor CSER ordering: KG-main (0.851) $>$ KG-2 (0.534) $>$
                KG-3 (0.380) $>$ KG-4 (0.254),
                broadly consistent with the heterogeneity hypothesis at the extremes.
                \textbf{Nuance}: KG-2 (same-model GPT-5.2$\times$GPT-5.2, CSER$=0.534$)
...
\textbf{Partially supported} & CSER ordering: KG-main(0.851)$>$KG-2(0.534)$>$KG-3(0.380)$>$KG-4(0.254); KG-2 (same-model, persona-diverse) outranks KG-3 (cross-vendor), suggesting persona opposition partially offsets same-model echo-chamber \\
Within-cell CSER trajectory is reproducible (same config $\to$ same CSER) &
\textbf{Early signal} & KG-2b CSER$=0.513$ tracking KG-2 CSER$=0.534$ (same echo-chamber trajectory, 72 nodes); KG-3b/4b trajectories pending 100+ cycles \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  \item \textbf{KG replication ($2\times2$ design, ongoing)}:
        KG-2 (GPT-5.2 $\times$ GPT-5.2) has accumulated 95 nodes (161 edges),
        showing early echo-chamber pressure: CSER declined from 1.00 (cycle~1)
        to 0.534. Replication-B (KG-2b, 72 nodes, CSER $=0.513$) tracks the same declining trajectory, providing
...
\begin{itemize}
  \item \textbf{Within-cell replication (early signal)}: Three replication-B instances
        ...
        were initiated 2026-03-05 and have reached 72 nodes (36 cycles).
        Current CSER readings (72 nodes each): KG-2b = 0.513, KG-3b = 0.475, KG-4b = 0.543.
        Notably, KG-2b (0.513) tracks KG-2's declining trajectory (current: 0.534, $\Delta=0.021$),
...
%      KG-2b CSER=0.513 tracking KG-2 CSER=0.534 (same declining trajectory, 72 nodes) (first within-cell replication signal confirmed).
...
ON win rate between 0.47 and 0.60 (high variance; see Figure~\ref{fig:persona_ablation_runs}).
