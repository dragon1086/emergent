...
\date{Draft v3.5 — Latest Snapshot (2026-03-05)}
...
\begin{enumerate}
  \item \textbf{Sample size ($N=4$ KG instances, $N=1$ per cell)}: Primary structural
        ...
        KG-2 & GPT-5.2 $\times$ GPT-5.2 & Same-model (OpenAI) & 0 & --- (재시작)$^{**}$ \\
        ...
        to GPT-5.2 $\times$ GPT-5.2 (zero capability gap, same model) to provide the
        strongest same-model echo-chamber test in the $2\times2$ matrix.
        Prior 30-cycle snapshot (CSER $=0.83$, GPT-4o $\times$ GPT-4o-mini) is retained
        as a supplementary data point.}
        \end{table}

        \noindent\textbf{Key replication finding ($N=4$ KG instances; KG-2: 30 cycles, KG-3/4: 130+ cycles)}:
        \begin{enumerate}
          ...
          \item Across-vendor CSER ordering: KG-main $>$ KG-2 $>$ KG-3 $>$ KG-4,
