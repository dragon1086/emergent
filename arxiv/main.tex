...
\begin{enumerate}
  ...
  \item \textbf{Reproducibility}: Not confirmed with different agent pairs.
        \textit{Partial replication (Cycle 85)}: The binary gate effect (Condition A,
        GCD problem) was confirmed across three independent LLMs ---
        Gemini 3 Flash, GPT-5.2, and Claude Sonnet 4.6 --- each achieving 5/5 pass rate.
        Full replication (different agent pair compositions, different KG structures)
        remains future work.
  ...
  \item \textbf{LLM generalizability}: Core experiments used Claude (Anthropic).
        \textit{Cycle 85 replication}: The binary gate effect was confirmed with
        Gemini 3 Flash (Google), GPT-5.2 (OpenAI), and Claude Sonnet 4.6 (Anthropic)
        --- all three achieving 5/5 pass rate on GCD under Condition A.
        Multi-LLM pair combinations (e.g., GPT-5.2 + Gemini co-evolution) remain
        unverified and are identified as primary future work.
\end{enumerate}
