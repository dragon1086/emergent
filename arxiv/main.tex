...
\date{Draft v3.3 — Latest Snapshot (2026-03-04)}
...
\begin{enumerate}
  \item \textbf{Sample size ($N=1$ system)}: This is a single-pair, single-KG, single-run
        ...
        original agent pair.
        \textit{Ongoing replication (2026-03-04)}: Three additional independent KG
        ...
        \midrule
        KG-main & Claude-sonnet-4-6 $\times$ GPT-next (undisclosed) & Cross & 1121 & 0.8510 \\
        KG-2 & GPT-4o $\times$ GPT-4o-mini & Same (OpenAI) & 30 & 0.8333 \\
        KG-3 & GPT-4o $\times$ Gemini-2.0-Flash & Cross & 30 & \textbf{0.9667} \\
        KG-4 & Gemini-Flash $\times$ Gemini-Pro & Same (Google) & 30 & 0.3667 \\
        \bottomrule
        \end{tabular}
        \caption*{\small KG-2/3/4: 30-cycle snapshots (2026-03-04).
        All four instances exceed the gate threshold ($\theta=0.30$).
        KG-4 (same-vendor Google) shows markedly lower CSER (0.37)
        vs.\ cross-vendor KG-3 (0.97), consistent with the heterogeneity hypothesis.
        KG-2 (same-vendor OpenAI) shows high CSER (0.83), suggesting within-vendor
        CSER depends on capability gap, not vendor identity alone.}
        \end{table}

        \noindent\textbf{Key replication finding ($N=4$ KG instances, 30 cycles each)}:
        \begin{enumerate}
          \item The gate-passing CSER result replicates across all four configurations
                (all CSER $\geq 0.30$), supporting generalizability of the gate mechanism.
          \item Cross-vendor (KG-3: 0.9667) $>$ same-vendor OpenAI (KG-2: 0.8333)
                $>$ same-vendor Google (KG-4: 0.3667), consistent with the theoretical
                prediction that architectural heterogeneity drives higher cross-source
                emergence --- though KG-4 remains above the gate.
          \item Within-vendor CSER magnitude depends on capability gap:
                GPT-4o $\times$ GPT-4o-mini (large capability gap) yields CSER $=0.83$,
                comparable to cross-vendor; Gemini Flash $\times$ Pro
                (smaller parameter gap within the same training paradigm) yields
                CSER $=0.37$, near the gate boundary.
        \end{enumerate}
...
\begin{enumerate}
  \item \textbf{KG replication ($2\times2$ design, completed 30 cycles)}:
        KG2/KG3/KG4 instances each completed 30 co-evolution cycles (2026-03-04).
        All four KG instances pass the CSER gate ($\geq 0.30$); cross-vendor
        KG-3 achieves the highest CSER (0.9667), while same-vendor Google
        KG-4 shows the lowest (0.3667), consistent with the heterogeneity
        hypothesis. See Limitations (Sec.~\ref{sec:limitations}) for full $N=4$ comparison table.
        Next target: $\geq 100$ cycles per instance for full trajectory analysis.
...
% ─────────────────────────────────────────────────────────────────────────────
\section*{Pre-Submission Critical Fixes (Author Checklist)}
% ─────────────────────────────────────────────────────────────────────────────

The following two items are identified as minimum-necessary actions before
formal venue submission. Both address concerns raised in pre-submission review
that cannot be resolved within the current single-experiment dataset.

\begin{enumerate}
  \item \textbf{Multi-seed / multi-pair replication plan}\\
        \textit{Issue}: All KG structural claims (CSER trajectories, $E_{v4}$
        values, D-063/D-064 pattern rates) derive from a single $N=1$ KG run.
        A reviewer cannot assess whether findings reflect systematic structural
        dynamics or experiment-specific artifacts.\\
        \textit{Required action}: Before submission, conduct at minimum
        \textbf{3 independent replications}: (a)~same agent pair, fresh KG seed;
        (b)~different agent pair (GPT-4o $\times$ Gemini), same domain;
        (c)~different domain (creative writing or scientific reasoning).
        Report mean $\pm$ std of CSER, $E_{v4}$, and paradoxical-crossing rate
        across all runs. If replication is not feasible before submission,
        reframe the paper explicitly as a \emph{single-system case study
        introducing a measurement framework}, removing any language that implies
        generalization.

  \item \textbf{Judge ensemble + statistical significance plan for amp benchmark}\\
        \textit{Issue}: The amp benchmark uses a single judge model (Gemini,
        $N=10$ questions). The $+9.5$ average insight gain and the
        70\%/30\% auto-persona preference result are based on one evaluator's
        ratings; no inter-rater reliability is reported; no significance test
        is provided.\\
        \textit{Required action}: Either (a)~add at minimum one additional
        independent judge (e.g., GPT-4o-mini as a second blind evaluator) and
        report inter-judge agreement (Cohen's $\kappa$ or Spearman $\rho$),
        or (b)~apply a one-sample $t$-test or Wilcoxon signed-rank test on the
        10 per-question insight-gain scores to compute a $p$-value and 95\%
        confidence interval. Without at least one of these, the amp results
        remain anecdotal and will be flagged by any statistical reviewer.
\end{enumerate}
...
Agent B (cokac) & \texttt{gpt-5.2-chat-latest} & OpenAI \\
...
\bottomrule
\end{tabular}
\caption{Model versions used in main experiments (data cutoff: 2026-03-04).}
\end{table}
