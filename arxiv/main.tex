...
KG-2 early trajectory (GPT-5.2 $\times$ GPT-5.2, 67 nodes): CSER $= 0.545$, declining from 1.00
...
KG-2b early CSER (0.549) closely matches the primary KG-2 (0.552), providing
...
\begin{enumerate}
  \item \textbf{Sample size ($N=4$ KG instances, $N=1$ per cell)}: Primary structural
        ...
        KG-2 & GPT-5.2 $\times$ GPT-5.2 & Same-model (OpenAI) & 116 & 0.552$^{**}$ \\
        KG-3 & GPT-next (undisclosed) $\times$ Gemini-2.0-Flash & Cross & 400 & \textbf{0.3800} \\
        KG-4 & Gemini-Flash $\times$ Gemini-Pro & Same (Google) & 402 & 0.2537$^*$ \\
        \bottomrule
        \end{tabular}
        ...
        Early trajectory (69 nodes, 116 edges): CSER declined from 1.00 (cycle 1)
        to 0.552 (current), consistent with progressive echo-chamber pressure under
        identical-model pairing. Replication-B (KG-2b, 32 nodes) already converges
        to CSER $= 0.549$, closely matching this value---first within-cell replication signal. Prior 30-cycle snapshot (CSER $=0.83$, GPT-4o $\times$
        ...
        \begin{enumerate}
          \item The gate-passing CSER result replicates across KG-main, KG-2, and KG-3
                ...
                echo-chamber pressure: CSER declined from 1.00 (cycle~1) to 0.552
                (69 nodes). Replication-B (KG-2b, 32 nodes, CSER $=0.549$) already
...
\begin{enumerate}
  \item \textbf{KG replication ($2\times2$ design, ongoing)}:
        KG-2 (GPT-5.2 $\times$ GPT-5.2) has accumulated 67 nodes (112 edges),
        showing early echo-chamber pressure: CSER declined from 1.00 (cycle~1)
        to 0.545. KG-3/KG-4 reached 130+ cycles (2026-03-05).
...
\begin{itemize}
  \item \textbf{Within-cell replication (early signal)}: Three replication-B instances
        ...
        Notably, KG-2b already converges to KG-2's established value (0.552),
...
Role & Model & Provider \\
\midrule
Agent A (Roki / yoki) & \texttt{claude-sonnet-4-6} & Anthropic \\
Agent B (cokac) & \texttt{gpt-next-undisclosed} & OpenAI \\
Amplification judge & \texttt{gemini-3-flash-preview} & Google \\
Cross-provider replication & \texttt{gpt-4o} + \texttt{gemini-2.0-flash} & OpenAI + Google \\
\bottomrule
\end{tabular}
