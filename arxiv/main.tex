...
Claims are limited to this experimental setting (탐색적 연구, N=5 벤치마크는 통계적 검정력 부족).
...
\noindent 이 분류체계는 검증 가능한 예측을 생성하는 이론이 아니라, 관찰을 정리하는 서술적 프레임워크이다.
(This taxonomy is a descriptive framework for organizing observations, not a theory that generates testable predictions.)
...
\begin{enumerate}
  \item \textbf{Single-system limitation (=1$ system, =4$ KG instances)}:
        ...
        {**}-2 CSER declining from 1.00 to 0.524 (99 nodes); replication-B (KG-2b, CSER/bin/zsh.497$) tracks same trajectory.
  ...
  \item \textbf{Statistical power (N=5 underpowered)}:
        ...
        Global CSER (/bin/zsh.8510$) is nearly identical to the Erd\H{o}s--R\'enyi random baseline
...
\begin{itemize}
  ...
  \item \textbf{Multi-LLM replication (Cycle 85)}: Binary gate effect replicated
        ...
        This provides cross-provider support for the gate (탐색적 결과, N=5, 통계적 검정력 부족);
  ...
  \item \textbf{Emergent $>$ Pipeline on logical reasoning (exploratory, N=5, underpowered)}: A three-way
        ...
        condition) provides \emph{예비 증거(preliminary evidence)} (탐색적 결과, N=5, 통계적 검정력 부족) that adversarial dual-path
...
\textbf{용어} & \textbf{정의} \\
\midrule
CSER & \textit{Cross-Source Edge Ratio}. KG 간선 중 서로 다른 에이전트 소스를 연결하는 간선의 비율. CSER $\in [0, 1]$. \\
\midrule
$E_{v4}$ & \textit{Emergence Score v4}. CSER, DCI, edge\_span\_norm, node\_age\_div의 가중 선형 결합 복합 지표. \\
\midrule
PES & \textit{Paradoxical Emergence Score}. span, cross\_source, tag\_overlap으로 계산되는 역설적 창발 점수. \\
\midrule
D-063 & \textit{Discovery \#63}. 직관에 반하는 교차출처 연결이 더 강한 창발을 생성하는 KG 구조 패턴. \\
\midrule
D-064 & \textit{Discovery \#64}. 미래 노드가 과거 노드를 소급적으로 재정의하는 현상 (span=160). \\
\midrule
D-077 & \textit{Discovery \#77}. CSER이 이진 게이트로 작동 ($\geq$0.30이면 품질 포화). \\
\midrule
DCI & \textit{Debate Convergence Index}. 미해결 논쟁 스레드 비율 측정. \\
\midrule
Condition A & 이질적 페르소나 조건 --- 서로 다른 역할의 에이전트. \\
\midrule
Condition B\_full & 동질적 페르소나 조건 --- 동일 역할의 에이전트 (20 사이클). \\
\midrule
Condition B\_partial & 부분적 동질 조건 --- 일부 속성만 공유. \\
\midrule
Condition C & 완전 동질 조건 --- 동일 에이전트 복제. \\
\midrule
L1--L5 & Layer 1(설계)--Layer 2(측정)--Layer 3(관찰)--Layer 4(행동)--Layer 5(창발) 분류 체계. \\
\midrule
pair\_designer & 에이전트 페르소나 자동 선택기 --- CSER 최적화 기반. \\
\bottomrule
\end{tabular}
\end{table}
...
CSER은 cross-source edge를 생성하도록 에이전트에게 지시하는 engineering choice를 반영할 수 있다.
CSER은 지시에 의한 것이든 자발적 창발에 의한 것이든 관계없이 구조적 결과를 측정한다
