...
\begin{enumerate}
  ...
  \item \textbf{Within-cell replication signal}: Independent replication-B instances
        ...
        KG-2b CSER$=0.508$ tracks KG-2 CSER$=0.534$ (same declining trajectory, $N=78$ nodes),
...
KG-2b CSER (0.508) tracks KG-2 (0.534) on the same declining trajectory, providing
...
were initiated 2026-03-05; KG-2b (78 nodes, CSER$=0.508$) tracks KG-2
...
\begin{itemize}
  ...
  \item \textbf{Within-cell replication}: Independent KG-2b instance (78 nodes) tracks
...
\begin{enumerate}
  \item \textbf{Sample size ($N=4$ KG instances, $N=1$ per cell)}: Primary structural
        ...
        (replication-B instances: KG-2b CSER$=0.508$ (tracking KG-2\,0.534), KG-3b CSER$=0.484$, KG-4b CSER$=0.552$; all at 78 nodes, 2026-03-05).
        ...
        identical-model pairing. Replication-B (KG-2b, 78 nodes, CSER $=0.508$) tracks the same declining
        ...
        \begin{enumerate}
          \item The gate-passing CSER result replicates across KG-main, KG-2, and KG-3
                ...
                (95 nodes). Replication-B (KG-2b, 78 nodes, CSER $=0.508$) tracks this declining trajectory---first within-cell replication of the same-model
                echo-chamber effect.
...
\textbf{Early signal} & KG-2b CSER$=0.508$ tracking KG-2 CSER$=0.534$ (same echo-chamber trajectory, 78 nodes); KG-3b/4b trajectories pending 100+ cycles \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  \item \textbf{KG replication ($2\times2$ design, ongoing)}:
        ...
        to 0.534. Replication-B (KG-2b, 78 nodes, CSER $=0.508$) tracks the same declining trajectory, providing
...
\begin{itemize}
  \item \textbf{Within-cell replication (early signal)}: Three replication-B instances
        ...
        were initiated 2026-03-05 and have reached 78 nodes (39 cycles).
        Current CSER readings (78 nodes each): KG-2b = 0.508, KG-3b = 0.484, KG-4b = 0.552.
        Notably, KG-2b (0.508) tracks KG-2's declining trajectory (current: 0.534, $\Delta=0.026$),
...
%      KG-2b CSER=0.508 tracking KG-2 CSER=0.534 (same declining trajectory, 78 nodes) (first within-cell replication signal confirmed).
