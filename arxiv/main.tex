...
\medskip\noindent\textbf{Data cutoff}: All KG metrics reported in this paper
use the snapshot dated \textbf{2026-03-02} unless otherwise noted. Experiments
labeled ``Cycle 86'' or ``Cycle 89'' reference historical snapshots taken at
those cycle counts; they are not updated to the latest snapshot.
Comparisons that span multiple snapshots are explicitly labeled with their
respective snapshot dates to ensure same-snapshot consistency.
...
\begin{enumerate}
  \item \textbf{Sample size ($N=4$ KG instances, $N=1$ per cell)}: Primary structural
        ...
        identical-model pairing. Replication-B (KG-2b, 36 nodes) already converges
        to CSER $= 0.552$, closely matching this value---first within-cell replication signal.
        ...
        \begin{enumerate}
          \item The gate-passing CSER result replicates across KG-main, KG-2, and KG-3
                ...
                (95 nodes). Replication-B (KG-2b, 68 nodes, CSER $=0.500$) tracks
                matches this trajectory---first within-cell replication of the same-model
                echo-chamber effect.
