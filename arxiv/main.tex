...
\begin{enumerate}
  \item \textbf{Sample size ($N=1$ system)}: This is a single-pair, single-KG, single-run
        ...
        instances have been initialized as part of a $2 \times 2$ vendor-diversity
        experiment: KG2 (GPT-4o + GPT-4o-mini, same-vendor OpenAI),
        KG3 (GPT-4o + Gemini-2.0-Flash, cross-vendor), and
        KG4 (Gemini-2.0-Flash + Gemini-2.0-Pro, same-vendor Google).
        These instances are in early evolution (cycle $\leq 5$) and are not
        reported as results in this paper; they form a pre-registered replication
        infrastructure targeting $N=4$ KG instances (main + 3) with a controlled
        vendor-diversity factor. Results will be reported in a follow-up.
