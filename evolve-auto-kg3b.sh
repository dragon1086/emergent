#!/bin/bash
# evolve-auto-kg3.sh — KG-3bb 자율 사이클 (cross-vendor: GPT-4o + Gemini Flash)
# 2×2 실험 설계: Google 계열 cross-vendor

REPO_DIR="$HOME/emergent"
KG3B_DIR="$HOME/emergent/kg3b"
KG3B_PATH="$KG3B_DIR/data/knowledge-graph.json"
LOG="$KG3B_DIR/logs/evolve-kg3b-$(date +%Y-%m-%d).log"
CYCLE_COUNT_FILE="/tmp/emergent-kg3b-cycles-$(date +%Y%m%d)"
MAX_CYCLES=100
OPENAI_KEY=$(grep "OPENAI_API_KEY" ~/.zshrc | head -1 | sed "s/.*='//;s/'.*//")
GEMINI_KEY=$(grep "GEMINI_API_KEY" ~/.zshrc | head -1 | sed "s/.*='//;s/'.*//")

mkdir -p "$KG3B_DIR/logs"
...
log "🌱 KG-3bb 사이클 시작 #$((COUNT + 1))/$MAX_CYCLES"

# 중복 방지
LOCK_FILE="/tmp/emergent-kg3b-running.lock"
if [[ -f "$LOCK_FILE" ]]; then
  LOCK_PID=$(cat "$LOCK_FILE" 2>/dev/null)
  if kill -0 "$LOCK_PID" 2>/dev/null; then
    log "⚠️  이전 KG-3bb 사이클 실행 중 — 스킵"
    exit 0
  fi
fi
echo $$ > "$LOCK_FILE"
...
PROMPT="당신은 emergent KG-3bb 실험의 Agent A (GPT-4o)입니다.
...
## 지시
1. KG-3bb에 추가할 의미있는 insight 또는 hypothesis 노드 1개를 제안하세요
...
resp = client.models.generate_content(model='gemini-2.5-flash', contents='KG-3bb 실험 Agent B (Gemini Flash)입니다. 다음 요청에 반박하거나 보완하세요 (한국어, 3문장 이내): $AGENT_B_REQUEST')
print(resp.text)
" 2>&1)
...
" "$NODE_LABEL" "$NODE_CONTENT" "${NODE_TYPE:-insight}" "${NODE_TAGS:-kg3,cross-vendor}" "${EDGE_TO:-}" "${EDGE_RELATION:-extends}" "$EDGE_LABEL" 2>/dev/null \
  | EMERGENT_KG_PATH="$KG3B_PATH" python3 src/add_node_safe.py 2>/dev/null)
...
# 메트릭 계산
EMERGENT_KG_PATH="$KG3B_PATH" python3 src/metrics.py 2>/dev/null | tail -5 | tee -a "$LOG" || true

# git 커밋
cd "$REPO_DIR"
git add kg3b/ 2>/dev/null
